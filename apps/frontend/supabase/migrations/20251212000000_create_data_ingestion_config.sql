CREATE TABLE public.data_ingestions (
    ...
    factory_type text NOT NULL,  -- 'REST', 'GRAPHQL', 'ARCHIVE2BQ'
    ...
    CONSTRAINT data_ingestions_factory_type_check CHECK (factory_type IN ('REST', 'GRAPHQL', 'ARCHIVE2BQ')),
...
CREATE POLICY "data_ingestions_org_access"
    ON public.data_ingestions AS PERMISSIVE FOR ALL
    USING (
        EXISTS (
            SELECT 1 FROM public.datasets
            WHERE datasets.id = data_ingestions.dataset_id
            AND EXISTS (
                SELECT 1 FROM public.users_by_organization
                WHERE users_by_organization.org_id = datasets.org_id
