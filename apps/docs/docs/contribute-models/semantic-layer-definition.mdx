...
attributes. We have chosen to model the `id` and `name` and `namespace`
dimensions as the attributes of the `artifact` model.
...
Dimensions can also be filtered on. Let's filter only for artifacts that are in the `GITHUB` namespace. We can do this by adding a filter to the query:

```python
query_with_namespace: QueryBuilder = registry.select(
    "artifact.id",
    "artifact.name",
).add_filter(
    "artifact.namespace = 'GITHUB'"
)
print(query_with_namespace.sql(pretty=True))
```
...
SELECT 
    ...
    artifact.artifact_name as artifact__name
FROM iceberg.oso.artifacts_v1 as artifact
WHERE artifact.artifact_namespace = 'GITHUB'
GROUP BY 1, 2
```
...
# This query will select the count of artifacts in a given namespace
...
```sql
SELECT
    artifact.artifact_namespace as artifact__namespace,
    count(artifact.artifact_id) as artifact__count
FROM iceberg.oso.artifacts_v1 as artifact
GROUP BY 1
```

Much like, dimensions, measures can also be filtered on. For instance, if we wanted
to only return artifact namespaces that have more than 100 artifacts, we could do the following:
