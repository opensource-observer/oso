...
:::note 
This guide is all about using the semantic layer and not defining it. If
you'd like to contribute to the semantic layer by defining additional models,
relationships, etc, see the guide
[here](../contribute-models/semantic-layer-definition.mdx)
:::
</AnatomyOfSemanticLayer>
...
_this is still not fully defined_

The semantic layer also provides a way to chain multiple queries together. This is akin to CTEs in SQL. 
...
query = oso.semantic.named_select(
  "github_release_metrics",
  "timeseries_metrics_by_project.sum as metric_sum",
  "project", # Without an attribute this will reference the model itself
).add_filter(
...
builder context automatically register that query as another model in an
