...
query = oso.semantic.select(
    ...
    "projects_by_collection.collection_name = 'ethereum-github'"
)
```
...
```python
query = oso.semantic.select(
    "timeseries_metrics_by_collection.sample_date",
    "timeseries_metrics_by_collection.amount",
).where(
    "collections.collection_name = 'ethereum-github'",
    "metrics.metric_name = 'GITHUB_commits_daily'"
)
```
...
query = oso.semantic.select(
    ...
    "collections.collection_name = 'op-retrofunding-4'",
    "metrics.metric_name = 'BASE_gas_fees_weekly'"
)
```
