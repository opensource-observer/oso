...
3. **Authentication:** "Prefer HttpOnly cookies with `credentials: "include"` in `HttpLink` options to avoid exposing tokens to JavaScript."
...
5. **Error Handling:** "Always handle `loading` and `error` states when using `useQuery` or `useLazyQuery`." For Suspense-based hooks, React manages loading through boundaries.
