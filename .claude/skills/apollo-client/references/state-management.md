...
**Important**: "Reactive variables store a single value that notifies `ApolloClient` instances when changed." In multi-user environments like SSR, avoid storing them as globals to prevent data leaks.
...
> **Note**: "`LocalState` is an Apollo Client 4.x concept and did not exist as a class in previous versions." In earlier versions, a `localState` option wasn't required.
...
"Local field `read` functions are defined in entity-level type policies." You can use reactive variables inside these functions, along with other calculations:
...
Query-level local fields can be defined using `LocalState` resolvers. **Note**: "Do not read reactive variables inside LocalState resolvers - this is not a documented/tested feature."
