...
export class SplitContractInvokedEvents1697488360720
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      insert into event("sourceId", "typeId", "time", "to", "from", "amount")
      select e."sourceId", (select c.id from cidc_et c), e."time", e."to", e."from", e."amount"
      from event e
    `);
    await queryRunner.query(`
      with cidf_et as (
      ...
      insert into event("sourceId", "typeId", "time", "to", "from", "amount")
      select e."sourceId", (select c.id from cidf_et c), e."time", e."to", e."from", CAST(e."size" as float)
      from event e
    `);
    await queryRunner.query(`
      delete from event e 
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      insert into event("sourceId", "typeId", "time", "to", "from", "amount", "size")
      select 
        count_e."sourceId", 
