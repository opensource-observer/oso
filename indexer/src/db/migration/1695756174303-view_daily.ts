...
export class ViewDaily1695756174303 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE MATERIALIZED VIEW "events_daily_by_artifact" WITH (timescaledb.continuous) AS 
    SELECT "toId",
      "type",
  ...
  `);
    ...
    await queryRunner.query(`CREATE MATERIALIZED VIEW "events_daily_by_project" WITH (timescaledb.continuous) AS 
    SELECT "projectId",
      "type",
      time_bucket(INTERVAL '1 day', "time") AS "bucketDaily",
      SUM(amount) as "amount"
