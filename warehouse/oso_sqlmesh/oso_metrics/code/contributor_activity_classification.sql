with first_of_activity_to_entity as (
  ...
  --   from_artifact_id,
  --   to_artifact_id
  -- from metrics.int_first_of_event_from_artifact
  ...
  from @metrics_entity_type_table(
    'metrics.int_first_contribution_to_{entity_type}'
...
lag_events_filtered as (
  ...
  from metrics.int_events_daily_to_artifact_with_lag as events
  where event_type in @activity_event_types
  group by bucket_day,
    event_source,
    from_artifact_id,
