...
with
    all_repos as (select * from @oso_source('bigquery.ossd.repositories')),

    all_ossd as (
        select * from @oso_source('bigquery.ossd.sbom') where artifact_source = 'GITHUB'
    )
...
where ao.artifact_namespace is null
