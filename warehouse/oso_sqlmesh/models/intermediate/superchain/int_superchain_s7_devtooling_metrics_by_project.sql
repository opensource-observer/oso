...
with
    trusted_developers as (
        select developer_id, sample_date from oso.int_superchain_s7_trusted_developers
    ),
    ...
    -- Repository metrics
    repo_metrics as (
        select repos.project_id, oso.metric_name, oso.amount, dev_graph.sample_date
        from oso.int_repositories_enriched as repos
        cross join
            (
                select 'stars' as metric_name, star_count as amount
                from oso.int_repositories_enriched
                union all
                select 'forks' as metric_name, fork_count as amount
                from oso.int_repositories_enriched
            ) as metrics
        -- Join to get sample dates from developer graph
        inner join
            oso.int_superchain_s7_project_to_developer_graph as dev_graph
            on repos.project_id = dev_graph.devtooling_project_id
        group by repos.project_id, oso.metric_name, oso.amount, dev_graph.sample_date
    ),
...
select sample_date, project_id, metric_name, amount
from
    (
        select *
