MODEL (
  name oso.int_events_daily__defillama_trading_volume,
  description 'Daily trading volume events from DefiLlama',
  kind FULL,
  dialect trino,
...
WITH all_volume_events AS (
  SELECT * FROM oso.stg_defillama__trading_volume_events
),

ranked_volume_events AS (
  SELECT *,
  ...
  FROM all_volume_events
),

deduplicated_volume_events AS (
  SELECT
    bucket_day,
    chain,
    slug,
...
volume_events_with_ids AS (
  ...
  FROM deduplicated_volume_events
)
