MODEL (
  name oso.int_superchain_s8_devtooling_deps_to_projects_graph,
...
@DEF(active_onchain_builder_date_threshold, DATE('2024-07-01'));

SELECT DISTINCT
  onchain_builders.project_id AS onchain_builder_project_id,
  devtools.project_id AS devtooling_project_id,
  code_deps.package_artifact_source AS dependency_source
FROM oso.int_code_dependencies AS code_deps
JOIN oso.int_superchain_s8_devtooling_onchain_builder_nodes AS onchain_builders
  ON code_deps.dependent_artifact_id = onchain_builders.repo_artifact_id
JOIN oso.int_superchain_s8_devtooling_repositories AS devtools
  ON code_deps.package_owner_artifact_id = devtools.repo_artifact_id
WHERE
