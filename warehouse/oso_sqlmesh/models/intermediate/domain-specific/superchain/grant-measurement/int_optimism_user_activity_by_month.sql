MODEL (
  name oso.int_optimism_user_activity_by_month,
  description 'User activity metrics on OP Mainnet by month',
  dialect trino,
  kind incremental_by_time_range(
    time_column bucket_month,
    batch_size 12,
    batch_concurrency 2,
    forward_only true,
    lookback @default_daily_incremental_lookback,
    auto_restatement_cron @default_auto_restatement_cron
  ),
  start @blockchain_incremental_start,
  cron '@monthly',
  partitioned_by MONTH("bucket_month"),
  grain (bucket_month, from_artifact_id),
  audits (
    has_at_least_n_rows(threshold := 0),
    no_gaps(
      time_column := bucket_month,
      no_gap_date_part := 'month',
    ),
  )
);


SELECT
  DATE_TRUNC('MONTH', time::DATE) AS bucket_month,
  from_artifact_id,
  from_artifact_name,
  SUM(l2_gas_fee / 1e18)::DOUBLE AS total_gas_fees,
  COUNT(*) AS total_transactions
FROM oso.int_events__superchain_transactions
WHERE
  time BETWEEN @start_dt AND @end_dt
GROUP BY 1, 2, 3