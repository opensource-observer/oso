...
with
    releases as (
        select artifact_id as repo_artifact_id, last_release_published
    ...
    packages as (
        ...
        group by package_github_owner, package_github_repo
    ),
...
left join releases on repos.artifact_id = releases.repo_artifact_id
left join
    packages
    on repos.artifact_namespace = packages.package_github_owner
    and repos.artifact_name = packages.package_github_repo
left join deps on repos.artifact_id = deps.dependency_artifact_id
