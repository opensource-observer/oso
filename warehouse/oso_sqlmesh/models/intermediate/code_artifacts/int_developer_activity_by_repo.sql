...
with
    developers as (
        ...
        inner join oso.int_users as users on events.from_artifact_id = users.user_id
        where
            events.event_type = 'COMMIT_CODE'
            and not regexp_matches(  -- TODO: confirm this will work in Trino too
                lower(users.display_name), '(^|[^a-z0-9_])bot([^a-z0-9_]|$)|bot$'
            )
    )
...
from oso.int_events__github as events
inner join developers on events.from_artifact_id = developers.developer_id
group by
