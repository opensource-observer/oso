model(
  name oso.int_artifacts_by_user,
  ...
  farcaster_users as (
    with
      farcaster as (
        ...
        inner join
          oso.stg_farcaster__addresses as farcaster_addresses
          on first_time_addresses.address = farcaster_addresses.address
        inner join
          oso.stg_farcaster__profiles as farcaster_profiles
...
select distinct
  ...
  artifact_namespace,
  artifact_name,
  @oso_id(user_source, user_namespace, user_name) as user_id,
  user_source_id,
  user_source,
  user_type,
  user_namespace,
