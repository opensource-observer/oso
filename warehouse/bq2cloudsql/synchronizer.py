...
class BigQueryCloudSQLSynchronizer(object):
        ...
        for config in self._configs:
            ...
            if config.mode.name == "INCREMENTAL_BY_PARTITION":
                if not table.time_partitioning:
                    print(
                        "Skipping table %s. Only time partitioning is supported for INCREMENTAL mode"
                        % source_table_id
                    )
                    continue
                else:
                    partitioning_type = table.time_partitioning.type_
                    queue, latest_partition_date = self.load_partition_queue(
                        config.source_table,
                        partitioning_type,
            ...
            try:
                ...
                print(
                    "Skipping table %s. It has unsupported columns %s"
                    % (source_table_id, e)
                )
                continue

            for partition in queue:
                self.extract_bq_table_to_gcs(
                    destination_prefix, config.source_table, partition
                )
    ...
    def extract_bq_table_to_gcs(
        ...
        print(
            "Exported {}:{}.{} to {}".format(
                self._project, self._dataset_id, table_id, destination_uri
            )
        )
    ...
    def commit_table(
        ...
        getattr(self, "commit_table_for_%s" % config.mode.name.lower())(
            temp_dest_table, config, last_partition_date
        )

    def commit_table_for_overwrite(
        self,
        temp_dest_table: str,
        config: TableSyncConfig,
        last_partition_date: pendulum.DateTime | None,
        ...
        with self._cloudsql.begin() as conn:
            ...
            insert_stmt = insert(self._sync_state_table).values(
                table_name=config.destination_table, **update_data
            )
            ...
            conn.execute(
                text(
                    f"ALTER TABLE {temp_dest_table} RENAME TO {config.destination_table}"
                )
            )

    def commit_table_for_incremental(
        self,
        temp_dest_table: str,
        config: TableSyncConfig,
        last_partition_date: pendulum.DateTime | None,
    ...
    def get_last_date_for_table(
        self, table_name: str
    ) -> Union[None, pendulum.DateTime]:
        with self._cloudsql.conn() as conn:
            result = conn.execute(
                select(self._sync_state_table).where(
                    self._sync_state_table.c.table_name == table_name
                )
            )
    ...
    def load_partition_queue(
        self, table_name: str, partitioning_type: str, full_refresh: bool = False
    ) -> Tuple[List[str], pendulum.DateTime | None]:
        ...
        for csv in self.list_csvs(prefix):
            ...
            if len(batch) == 32:
                ...
                destination_filename = (
                    f"{prefix}/{dest_filename_prefix}{batch_count:010d}.csv"
                )
    ...
    def combine_and_delete_csvs(
        self, bucket: Bucket, source_paths: List[str], destination_path: str
    ):
