...
class BasicText2SQL(
    ...
    async def handle_text2sql_query(
        self, ctx: Context, event: Text2SQLStartEvent
    ) -> Text2SQLGenerationEvent:
        ...
        try:
            ...
            logger.debug("query engine called successfully for query[%s]", event.id)
        except Exception as e:
            logger.error("Error calling query engine tool query[%s]: %s", event.id, e)
            raise ValueError(
                f"Failed to call query engine tool query[{event.id}]: {e}"
            ) from e
