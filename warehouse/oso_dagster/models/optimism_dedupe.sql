...
SELECT blocks.block_timestamp, {{ source(worker_table).sql_columns(prefix="traces", exclude=["_checkpoint", "block_timestamp"]) }}
FROM {{ source(worker_table).sql_from() }} AS traces
