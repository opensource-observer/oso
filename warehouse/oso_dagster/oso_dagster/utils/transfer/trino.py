...
from oso_dagster.utils.transfer.base import Exporter
from oso_dagster.utils.transfer.storage import (
    TimeOrderedStorage,
    TimeOrderedStorageFile,
)
...
class TrinoExporter(Exporter):
    ...
    def process_columns(
        ...
        assert isinstance(column_type, exp.DataType), (
    ...
    def export_path(self, export_time: datetime, export_table_name: str):
        return self.time_ordered_storage.generate_path(
            export_time,
            f"{export_table_name}/",  # ensure trailing slash
        )
