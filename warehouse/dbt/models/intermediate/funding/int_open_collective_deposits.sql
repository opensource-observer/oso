with open_collective_deposits as (
  select
    ...
    LOWER(JSON_EXTRACT_SCALAR(to_account, '$.slug')) as to_project_slug,
    UPPER(JSON_EXTRACT_SCALAR(to_account, '$.type')) as to_project_type,
...
select
  ...
  projects.project_id,
  open_collective_deposits.to_project_name,
  open_collective_deposits.to_project_slug,
  open_collective_deposits.to_project_type,
  open_collective_deposits.currency,
  open_collective_deposits.amount
from open_collective_deposits
inner join {{ ref('int_open_collective_projects') }} as projects
  on open_collective_deposits.to_project_slug = projects.artifact_source_id
