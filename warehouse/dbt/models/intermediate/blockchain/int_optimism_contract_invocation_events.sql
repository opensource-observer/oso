select -- noqa: ST06
  ...
  COALESCE(to_artifacts.artifact_type, "UNKNOWN_ADDRESS") as to_type,
  ...
  COALESCE(from_artifacts.artifact_type, "UNKNOWN_ADDRESS") as from_type,
...
left join {{ ref('int_ossd__artifacts_by_project') }} as to_artifacts
  on traces.to_address = to_artifacts.artifact_source_id
left join {{ ref('int_ossd__artifacts_by_project') }} as from_artifacts
