...
class OptimismAttestations(BaseModel):
    ...
    decoded_data_json: str = Column(
        "TEXT", description="Decoded JSON data of the attestation"
    )
    recipient: str = Column(
        "VARCHAR", description="The recipient address of the attestation"
    )
    ...
    time_created: int = Column(
        "INTEGER", description="Timestamp when the attestation was created"
    )
    ...
    is_offchain: bool = Column(
        "BOOLEAN", description="Whether the attestation is offchain"
    )
