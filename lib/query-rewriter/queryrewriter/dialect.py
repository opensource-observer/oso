...
This is forked directly from sqlmesh.core.dialect but removes sqlmesh parsers
for additional stanzas like `MODEL`, `AUDIT`, `METRIC`, etc this is under the
following LICENSE:
"""
...
from queryrewriter.errors import ConfigError, DialectError
from queryrewriter.pandasutils import columns_to_types_from_df
...
def _parse_id_var(
    ...
    if (
        self._prev
        and self._prev.text == SQLMESH_MACRO_PREFIX
        and self._match(TokenType.L_BRACE)
    ):
    ...
    while (
        ...
        identifier = self.expression(
            exp.Identifier, this=this, quoted=identifier.quoted
        )
...
def _parse_macro(self: Parser, keyword_macro: str = "") -> t.Optional[exp.Expression]:
    ...
    field = (
        self._parse_primary()
        or self._parse_function(functions={})
...
def _parse_with(
    self: Parser, skip_with_token: bool = False
) -> t.Optional[exp.Expression]:
...
def _parse_join(
    ...
    if not macro:
        ...
        return self.__parse_join(
            skip_join_token=skip_join_token, parse_bracket=parse_bracket
        )  # type: ignore
...
def _parse_where(
    self: Parser, skip_where_token: bool = False
) -> t.Optional[exp.Expression]:
...
def _parse_group(
    self: Parser, skip_group_by_token: bool = False
) -> t.Optional[exp.Expression]:
...
def _parse_having(
    self: Parser, skip_having_token: bool = False
) -> t.Optional[exp.Expression]:
...
def _parse_limit(
    ...
    macro.this.append(
        "expressions", self.__parse_limit(this, top=top, skip_limit_token=True)
    )  # type: ignore
    return macro
...
def _parse_macro_or_clause(
    self: Parser, parser: t.Callable
) -> t.Optional[exp.Expression]:
...
def _parse_props(self: Parser) -> t.Optional[exp.Expression]:
    ...
    if name == "time_data_type":
        ...
        value = self.expression(
            exp.Tuple, expressions=self._parse_csv(self._parse_equality)
        )
        self._match_r_paren()
    else:
...
def _sqlmesh_ddl_sql(
    self: Generator, expression: Model | Audit | Metric, name: str
) -> str:
...
def format_model_expressions(
    ...
    **kwargs: t.Any,
    ...
    return ";\n\n".join(
        expression.sql(pretty=True, dialect=dialect, **kwargs)
        for expression in expressions
    ).strip()
...
def parse_one(
    ...
    if not expressions:
        raise DialectError(f"No expressions found in '{sql}'")
    elif len(expressions) > 1:
        raise DialectError(f"Multiple expressions found in '{sql}'")
...
def parse(
    ...
    def parse_sql_chunk(
        chunk: t.List[Token], meta_sql: bool = True
    ) -> t.List[exp.Expression]:
        parsed_expressions: t.List[t.Optional[exp.Expression]] = (
            parser.parse(chunk, sql)
            if into is None
            else parser.parse_into(into, chunk, sql)
        )
        expressions = []
    ...
    def parse_jinja_chunk(
        chunk: t.List[Token], meta_sql: bool = True
    ) -> exp.Expression:
        start, *_, end = chunk
        segment = sql[start.end + 2 : end.start - 1]
        factory = (
            jinja_query if chunk_type == ChunkType.JINJA_QUERY else jinja_statement
        )
    ...
    def parse_virtual_statement(
        ...
        while (
            chunks[pos - 1][0] == []
            or chunks[pos - 1][0][-1].text.upper() != ON_VIRTUAL_UPDATE_END
        ):
            ...
            if chunk_type == ChunkType.JINJA_STATEMENT:
                ...
                virtual_update_statements.extend(
                    parse_sql_chunk(
                        chunk[
                            int(chunk[0].text.upper() == ON_VIRTUAL_UPDATE_BEGIN) : -1
                        ],
                        False,
                    ),
                )
...
def extend_sqlglot() -> None:
    ...
    for dialect in Dialect.classes.values():
        ...
        if hasattr(dialect, "Tokenizer"):
            tokenizers.add(dialect.Tokenizer)  # type: ignore
        if hasattr(dialect, "Parser"):
            parsers.add(dialect.Parser)  # type: ignore
        if hasattr(dialect, "Generator"):
            generators.add(dialect.Generator)  # type: ignore
    ...
    for parser in parsers:
        parser.FUNCTIONS.update(
            {"JINJA": Jinja.from_arg_list, "METRIC": MetricAgg.from_arg_list}
        )
            ...
            generator.TRANSFORMS.update(
                {
                    ...
                    DColonCast: lambda self,
                    e: f"{self.sql(e, 'this')}::{self.sql(e, 'to')}",
                    Jinja: lambda self, e: e.name,
                    JinjaQuery: lambda self,
                    e: f"{JINJA_QUERY_BEGIN};\n{e.name}\n{JINJA_END};",
                    JinjaStatement: lambda self,
                    e: f"{JINJA_STATEMENT_BEGIN};\n{e.name}\n{JINJA_END};",
                    VirtualUpdateStatement: lambda self, e: _on_virtual_update_sql(
                        self, e
                    ),
                    MacroDef: lambda self,
                    e: f"@DEF({self.sql(e.this)}, {self.sql(e.expression)})",
...
def select_from_values_for_batch_range(
    ...
    if not values:
        ...
        expressions = [
            tuple(
                exp.cast(exp.null(), to=kind)
                for kind in source_columns_to_types.values()
            )
        ]
    else:
        where = None
    ...
    casted_columns = [
        exp.alias_(
            exp.cast(
                exp.column(column) if column in source_columns_to_types else exp.Null(),
                to=kind,
            ),
            column,
            copy=False,
...
def normalize_model_name(
    ...
    if isinstance(table, exp.Column):
        table = exp.table_(
            table.this, db=table.args.get("table"), catalog=table.args.get("db")
        )
    else:
    ...
    if TABLES_META not in expression.meta:
        expression.meta[TABLES_META] = {
            normalize_model_name(
                table, default_catalog=default_catalog, dialect=dialect
            )
            for scope in traverse_scope(expression)
            for table in scope.tables  # type: ignore
...
def transform_values(
    ...
    def _transform_value(value: t.Any, dtype: exp.DataType) -> t.Any:
        ...
        if (
            ...
            expressions = []
            for (field_name, field_value), field_type in zip(
                value.items(), dtype.expressions
            ):
...
def _unquote_schema(schema: t.Dict) -> t.Dict:
    ...
    return {
        k.strip('"'): _unquote_schema(v) if isinstance(v, dict) else v
...
def interpret_expression(
    e: exp.Expression,
) -> exp.Expression | str | int | float | bool:
    if e.is_int:
        return int(e.this)
    if e.is_number:
        return float(e.this)
...
def extract_function_calls(func_calls: t.Any, allow_tuples: bool = False) -> t.Any:
    ...
    if isinstance(func_calls, (exp.Tuple, exp.Array)):
        return [
            extract_func_call(i, allow_tuples=allow_tuples)
            for i in func_calls.expressions
        ]
    ...
    if isinstance(func_calls, list):
        ...
        for entry in func_calls:
            if isinstance(entry, dict):
                ...
                raise ConfigError(
                    f"Audit must be a dictionary or named tuple. Got {entry}."
                )
